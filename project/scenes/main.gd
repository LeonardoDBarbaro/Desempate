...
func new_game():
	...
	$GameOverMenu.hide()
	score = 0
	$Hud.get_node("ScoreLabel").text = "PUNTOS: " + str(score)
	move_direction = up
	can_move = true
	generate_snake()
	move_food()
...
func _on_game_over_menu_restart():
	...
	var array1
	array1 = global_singleton.get_global_data()
	var data_to_send = {str(array1[1]):str(array1[0]) + '-' + str(gege)}
	print(data_to_send)
	var json = JSON.stringify(data_to_send)
	var headers = ["Content-Type: application/json"]
	var url = "http://190.55.215.55:25565/palta"
	if array1[1] == "sol":
		url = "http://190.55.215.55:25565/sol"
	$HTTPRequest.request(url,true, HTTPClient.METHOD_POST, json)
